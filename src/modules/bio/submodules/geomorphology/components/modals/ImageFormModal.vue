...
const canSubmit = computed(() => {
  ...
  return !!form.value.profile_id && !!file.value
})

function reset() {
  error.value = null
  form.value = { profile_id: null, description: '' }
  file.value = null
}
...
function handleSubmit() {
  error.value = null
  if (!canSubmit.value) { error.value = props.isEdit ? 'Заполните обязательные поля' : 'Выберите профиль и файл'; return }
  isSubmitting.value = true
  let payload
  let isMultipart = false
  if (!props.isEdit || file.value) {
    const fd = new FormData()
    if (form.value.profile_id != null) fd.append('profile', form.value.profile_id)
    if (file.value) fd.append('file', file.value)
    if (form.value.description) fd.append('description', form.value.description)
    payload = fd
    isMultipart = true
  } else {
    payload = { profile: form.value.profile_id, description: form.value.description || undefined }
  }
