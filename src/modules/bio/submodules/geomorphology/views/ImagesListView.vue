...
import ImageFormModal from '@/modules/bio/submodules/geomorphology/components/modals/ImageFormModal.vue'
...
const createModalId = 'g-img-create'
const editModalId = 'g-img-edit'
const deleteModalId = 'g-img-delete'
...
const fetchItems = async () => {
  ...
  error.value = null
  try {
    const r = await apiClient.get(endpoints.bio.geomorphology.images)
...
const handleDelete = async () => {
  ...
  try {
    const r = await apiClient.delete(endpoints.bio.geomorphology.imageDetail(itemToDelete.value.id))
    if (r.success) {
...
const handleCreate = async (payload, cb, isMultipart) => {
  try {
    const r = await apiClient.post(endpoints.bio.geomorphology.images, payload)
    if (r.success) {
...
const handleUpdate = async (payload, cb, isMultipart) => {
  ...
  try {
    const r = await apiClient.put(endpoints.bio.geomorphology.imageDetail(itemToEdit.value.id), payload)
    if (r.success) {
...
<template>
  <div class="card p-0">
    <div class="card-header custom-card-header border border-bottom-0 text-white d-flex justify-content-between align-items-center">
      <h5 class="mb-0">Геоморфология — изображения</h5>
