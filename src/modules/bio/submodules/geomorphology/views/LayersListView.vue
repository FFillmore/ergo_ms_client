...
import LayerFormModal from '@/modules/bio/submodules/geomorphology/components/modals/LayerFormModal.vue'
...
const createModalId = 'g-layer-create'
const editModalId = 'g-layer-edit'
const deleteModalId = 'g-layer-delete'
...
const columns = [
  ...
  { field: 'depth_from', header: 'От', sortable: true },
  { field: 'depth_to', header: 'До', sortable: true },
  { field: 'color', header: 'Цвет', sortable: true },
  { field: 'texture', header: 'Текстура', sortable: true },
  { field: 'ph_level', header: 'pH', sortable: true },
]
...
const filtered = computed(() => {
  ...
  return items.value.filter((x) =>
    String(x.profile || '').toLowerCase().includes(term) ||
    String(x.color || '').toLowerCase().includes(term) ||
    String(x.texture || '').toLowerCase().includes(term)
  )
})
...
const fetchItems = async () => {
  ...
  error.value = null
  try {
    const r = await apiClient.get(endpoints.bio.geomorphology.layers)
    const list = r.success ? (r.data.results || r.data || []) : []
    items.value = list.map(x => ({ ...x, id: x.id ?? x.pk }))
    if (!r.success) error.value = handleApiError(r, 'Ошибка загрузки слоев')
  } catch (e) {
    error.value = handleApiError(e, 'Ошибка загрузки слоев')
  } finally {
...
const handleDelete = async () => {
  ...
  try {
    const r = await apiClient.delete(endpoints.bio.geomorphology.layerDetail(itemToDelete.value.id))
    if (r.success) {
      toast.success('Слой удален')
      await fetchItems()
      closeById(deleteModalId)
    } else {
      handleApiError(r, 'Ошибка удаления слоя', toast)
    }
  } catch (e) {
    handleApiError(e, 'Ошибка удаления слоя', toast)
  } finally {
...
const handleCreate = async (payload, cb) => {
  try {
    const r = await apiClient.post(endpoints.bio.geomorphology.layers, payload)
    if (r.success) {
      toast.success('Слой создан')
      await fetchItems()
      closeById(createModalId)
      cb && cb(null)
    } else {
      const err = handleApiError(r, 'Ошибка создания слоя', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка создания слоя', toast)
    cb && cb(err)
  }
}
...
const handleUpdate = async (payload, cb) => {
  ...
  try {
    const r = await apiClient.put(endpoints.bio.geomorphology.layerDetail(itemToEdit.value.id), payload)
    if (r.success) {
      toast.success('Слой обновлен')
      await fetchItems()
      closeById(editModalId)
      cb && cb(null)
    } else {
      const err = handleApiError(r, 'Ошибка обновления слоя', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка обновления слоя', toast)
    cb && cb(err)
  }
}
...
<template>
  <div class="card p-0">
    <div class="card-header custom-card-header border border-bottom-0 text-white d-flex justify-content-between align-items-center">
      <h5 class="mb-0">Геоморфология — слои</h5>
      <div class="d-flex gap-2">
        ...
        <button class="btn btn-sm btn-primary text-nowrap" @click="openCreate">
          <Plus :size="16" class="me-1" /> Добавить слой
        </button>
      </div>
    </div>
    ...
    <DeleteConfirmModal :modalId="deleteModalId" :title="`Подтвердите удаление слоя`" :message="`Вы уверены, что хотите удалить слой?`" :loading="isDeleting" warningText="Это действие нельзя будет отменить." @confirm="handleDelete" />

    <LayerFormModal :modalId="createModalId" @create="handleCreate" />
    <LayerFormModal :modalId="editModalId" :initialData="itemToEdit" :isEdit="true" @update="handleUpdate" />
  </div>
</template>
