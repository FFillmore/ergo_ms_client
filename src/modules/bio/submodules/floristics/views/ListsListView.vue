...
import ListFormModal from '@/modules/bio/submodules/floristics/components/modals/ListFormModal.vue'
...
const createModalId = 'fl-list-create'
const editModalId = 'fl-list-edit'
const deleteModalId = 'fl-list-delete'
...
const columns = [
  ...
  { field: 'description', header: 'Описание', sortable: false },
]

const filtered = computed(() => {
  if (!search.value) return items.value
  const term = search.value.toLowerCase()
  return items.value.filter((x) => String(x.name || '').toLowerCase().includes(term))
})
...
const fetchItems = async () => {
  ...
  error.value = null
  try {
    const r = await apiClient.get(endpoints.bio.floristics.lists)
    const list = r.success ? (r.data.results || r.data || []) : []
    items.value = list.map(x => ({ ...x, id: x.id ?? x.pk }))
    if (!r.success) error.value = handleApiError(r, 'Ошибка загрузки списков')
  } catch (e) {
    error.value = handleApiError(e, 'Ошибка загрузки списков')
  } finally {
...
const handleDelete = async () => {
  ...
  try {
    const r = await apiClient.delete(endpoints.bio.floristics.listDetail(itemToDelete.value.id))
    if (r.success) {
      toast.success('Список удален')
      await fetchItems()
      closeById(deleteModalId)
    } else {
      handleApiError(r, 'Ошибка удаления списка', toast)
    }
  } catch (e) {
    handleApiError(e, 'Ошибка удаления списка', toast)
  } finally {
...
const handleCreate = async (payload, cb) => {
  try {
    const r = await apiClient.post(endpoints.bio.floristics.lists, payload)
    if (r.success) {
      ...
      const err = handleApiError(r, 'Ошибка создания списка', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка создания списка', toast)
    cb && cb(err)
  }
}
...
const handleUpdate = async (payload, cb) => {
  ...
  try {
    const r = await apiClient.put(endpoints.bio.floristics.listDetail(itemToEdit.value.id), payload)
    if (r.success) {
      ...
      const err = handleApiError(r, 'Ошибка обновления списка', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка обновления списка', toast)
    cb && cb(err)
  }
}
...
<template>
  <div class="card p-0">
    <div class="card-header custom-card-header border border-bottom-0 text-white d-flex justify-content-between align-items-center">
      <h5 class="mb-0">Флористика — списки</h5>
      <div class="d-flex gap-2">
        ...
        <button class="btn btn-sm btn-primary text-nowrap" @click="openCreate">
          <Plus :size="16" class="me-1" /> Добавить список
        </button>
      </div>
    </div>
    ...
    <DeleteConfirmModal :modalId="deleteModalId" :title="`Подтвердите удаление списка`" :message="`Вы уверены, что хотите удалить список?`" :loading="isDeleting" warningText="Это действие нельзя будет отменить." @confirm="handleDelete" />

    <ListFormModal :modalId="createModalId" @create="handleCreate" />
    <ListFormModal :modalId="editModalId" :initialData="itemToEdit" :isEdit="true" @update="handleUpdate" />
  </div>
</template>
