...
import ItemFormModal from '@/modules/bio/submodules/floristics/components/modals/ItemFormModal.vue'
...
const createModalId = 'fl-item-create'
const editModalId = 'fl-item-edit'
const deleteModalId = 'fl-item-delete'
const isDeleting = ref(false)
const itemToEdit = ref(null)
const itemToDelete = ref(null)

const columns = [
  { field: 'list_name', header: 'Список', sortable: true },
...
const filtered = computed(() => {
  ...
  return items.value.filter((x) =>
    String(x.species || '').toLowerCase().includes(term) ||
    String(x.list || '').toLowerCase().includes(term) ||
...
const fetchItems = async () => {
  ...
  error.value = null
  try {
    const r = await apiClient.get(endpoints.bio.floristics.items)
    const list = r.success ? (r.data.results || r.data || []) : []
    items.value = list.map(x => ({ ...x, id: x.id ?? x.pk }))
    if (!r.success) error.value = handleApiError(r, 'Ошибка загрузки элементов')
  } catch (e) {
    error.value = handleApiError(e, 'Ошибка загрузки элементов')
  } finally {
...
const handleDelete = async () => {
  ...
  try {
    const r = await apiClient.delete(endpoints.bio.floristics.itemDetail(itemToDelete.value.id))
    if (r.success) {
      toast.success('Элемент удален')
      await fetchItems()
      closeById(deleteModalId)
    } else {
      handleApiError(r, 'Ошибка удаления элемента', toast)
    }
  } catch (e) {
    handleApiError(e, 'Ошибка удаления элемента', toast)
  } finally {
...
const handleCreate = async (payload, cb) => {
  try {
    const r = await apiClient.post(endpoints.bio.floristics.items, payload)
    if (r.success) {
      ...
      const err = handleApiError(r, 'Ошибка создания элемента', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка создания элемента', toast)
    cb && cb(err)
  }
}
...
const handleUpdate = async (payload, cb) => {
  ...
  try {
    const r = await apiClient.put(endpoints.bio.floristics.itemDetail(itemToEdit.value.id), payload)
    if (r.success) {
      ...
      const err = handleApiError(r, 'Ошибка обновления элемента', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка обновления элемента', toast)
    cb && cb(err)
  }
}
...
<template>
  <div class="card p-0">
    <div class="card-header custom-card-header border border-bottom-0 text-white d-flex justify-content-between align-items-center">
      <h5 class="mb-0">Флористика — элементы</h5>
      <div class="d-flex gap-2">
        ...
        <button class="btn btn-sm btn-primary text-nowrap" @click="openCreate">
          <Plus :size="16" class="me-1" /> Добавить элемент
        </button>
      </div>
    </div>
    ...
    <DeleteConfirmModal :modalId="deleteModalId" :title="`Подтвердите удаление элемента`" :message="`Вы уверены, что хотите удалить элемент?`" :loading="isDeleting" warningText="Это действие нельзя будет отменить." @confirm="handleDelete" />

    <ItemFormModal :modalId="createModalId" @create="handleCreate" />
    <ItemFormModal :modalId="editModalId" :initialData="itemToEdit" :isEdit="true" @update="handleUpdate" />
  </div>
</template>
