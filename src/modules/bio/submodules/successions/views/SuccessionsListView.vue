...
import SuccessionFormModal from '@/modules/bio/submodules/successions/components/modals/SuccessionFormModal.vue'
...
const createModalId = 'succ-create'
const editModalId = 'succ-edit'
const deleteModalId = 'succ-delete'
...
const columns = [
  ...
  { field: 'succession_type', header: 'Тип', sortable: true },
...
const filtered = computed(() => {
  ...
  return items.value.filter((x) =>
    ...
    String(x.succession_type || '').toLowerCase().includes(term) ||
...
const fetchItems = async () => {
  ...
  error.value = null
  try {
    const r = await apiClient.get(endpoints.bio.successions.successions)
    const list = r.success ? (r.data.results || r.data || []) : []
    items.value = list.map(x => ({ ...x, id: x.id ?? x.pk }))
    if (!r.success) error.value = handleApiError(r, 'Ошибка загрузки сукцессий')
  } catch (e) {
    error.value = handleApiError(e, 'Ошибка загрузки сукцессий')
  } finally {
...
const handleDelete = async () => {
  ...
  try {
    const r = await apiClient.delete(endpoints.bio.successions.successionDetail(itemToDelete.value.id))
    if (r.success) {
      toast.success('Сукцессия удалена')
      await fetchItems()
      closeById(deleteModalId)
    } else {
      handleApiError(r, 'Ошибка удаления сукцессии', toast)
    }
  } catch (e) {
    handleApiError(e, 'Ошибка удаления сукцессии', toast)
  } finally {
...
const handleCreate = async (payload, cb) => {
  try {
    const r = await apiClient.post(endpoints.bio.successions.successions, payload)
    if (r.success) {
      toast.success('Сукцессия создана')
      await fetchItems()
      closeById(createModalId)
      cb && cb(null)
    } else {
      const err = handleApiError(r, 'Ошибка создания сукцессии', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка создания сукцессии', toast)
    cb && cb(err)
  }
}
...
const handleUpdate = async (payload, cb) => {
  ...
  try {
    const r = await apiClient.put(endpoints.bio.successions.successionDetail(itemToEdit.value.id), payload)
    if (r.success) {
      toast.success('Сукцессия обновлена')
      await fetchItems()
      closeById(editModalId)
      cb && cb(null)
    } else {
      const err = handleApiError(r, 'Ошибка обновления сукцессии', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка обновления сукцессии', toast)
    cb && cb(err)
  }
}
...
<template>
  <div class="card p-0">
    <div class="card-header custom-card-header border border-bottom-0 text-white d-flex justify-content-between align-items-center">
      ...
      <div class="d-flex gap-2">
        ...
        <button class="btn btn-sm btn-primary text-nowrap" @click="openCreate">
          <Plus :size="16" class="me-1" /> Добавить сукцессию
        </button>
      </div>
    </div>
    ...
    <DeleteConfirmModal :modalId="deleteModalId" :title="`Подтвердите удаление сукцессии`" :message="`Вы уверены, что хотите удалить сукцессию?`" :loading="isDeleting" warningText="Это действие нельзя будет отменить." @confirm="handleDelete" />

    <SuccessionFormModal :modalId="createModalId" @create="handleCreate" />
    <SuccessionFormModal :modalId="editModalId" :initialData="itemToEdit" :isEdit="true" @update="handleUpdate" />
  </div>
</template>
