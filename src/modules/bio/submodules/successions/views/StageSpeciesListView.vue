...
import StageSpeciesFormModal from '@/modules/bio/submodules/successions/components/modals/StageSpeciesFormModal.vue'
...
const createModalId = 'ss-create'
const editModalId = 'ss-edit'
const deleteModalId = 'ss-delete'
const isDeleting = ref(false)
const itemToEdit = ref(null)
const itemToDelete = ref(null)

const columns = [
  { field: 'stage_number', header: 'Стадия', sortable: true },
  { field: 'plant_species_title', header: 'Вид растения', sortable: true },
  { field: 'abundance', header: 'Обилие', sortable: true },
]
...
const filtered = computed(() => {
  ...
  return items.value.filter((x) =>
    ...
    String(x.stage || '').toLowerCase().includes(term) ||
    String(x.abundance || '').toLowerCase().includes(term)
  )
})
...
const fetchItems = async () => {
  ...
  error.value = null
  try {
    const r = await apiClient.get(endpoints.bio.successions.stageSpecies)
    const list = r.success ? (r.data.results || r.data || []) : []
    items.value = list.map(x => ({ ...x, id: x.id ?? x.pk }))
    if (!r.success) error.value = handleApiError(r, 'Ошибка загрузки видов стадий')
  } catch (e) {
    error.value = handleApiError(e, 'Ошибка загрузки видов стадий')
  } finally {
...
const handleDelete = async () => {
  ...
  try {
    const r = await apiClient.delete(endpoints.bio.successions.stageSpeciesDetail(itemToDelete.value.id))
    if (r.success) {
      toast.success('Запись удалена')
      await fetchItems()
      closeById(deleteModalId)
    } else {
      handleApiError(r, 'Ошибка удаления записи', toast)
    }
  } catch (e) {
    handleApiError(e, 'Ошибка удаления записи', toast)
  } finally {
...
const handleCreate = async (payload, cb) => {
  try {
    const r = await apiClient.post(endpoints.bio.successions.stageSpecies, payload)
    if (r.success) {
      ...
      const err = handleApiError(r, 'Ошибка создания записи', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка создания записи', toast)
    cb && cb(err)
  }
}
...
const handleUpdate = async (payload, cb) => {
  ...
  try {
    const r = await apiClient.put(endpoints.bio.successions.stageSpeciesDetail(itemToEdit.value.id), payload)
    if (r.success) {
      ...
      const err = handleApiError(r, 'Ошибка обновления записи', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка обновления записи', toast)
    cb && cb(err)
  }
}
...
<template>
  <div class="card p-0">
    <div class="card-header custom-card-header border border-bottom-0 text-white d-flex justify-content-between align-items-center">
      <h5 class="mb-0">Виды на стадиях</h5>
      <div class="d-flex gap-2">
        ...
        <button class="btn btn-sm btn-primary text-nowrap" @click="openCreate">
          <Plus :size="16" class="me-1" /> Добавить запись
        </button>
      </div>
    </div>
    ...
    <DeleteConfirmModal :modalId="deleteModalId" :title="`Подтвердите удаление записи`" :message="`Вы уверены, что хотите удалить запись?`" :loading="isDeleting" warningText="Это действие нельзя будет отменить." @confirm="handleDelete" />

    <StageSpeciesFormModal :modalId="createModalId" @create="handleCreate" />
    <StageSpeciesFormModal :modalId="editModalId" :initialData="itemToEdit" :isEdit="true" @update="handleUpdate" />
  </div>
</template>
