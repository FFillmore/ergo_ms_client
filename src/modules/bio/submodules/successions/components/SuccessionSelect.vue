...
const props = defineProps({
  ...
  placeholder: { type: String, default: 'Выберите сукцессию' },
})

const emit = defineEmits(['update:modelValue', 'change'])

const fetchSuccessions = async (query, page) => {
  const params = { page }
  if (query) params.search = query
  try {
    const r = await apiClient.get(endpoints.bio.successions.successions, params)
    let list = r?.data?.results || r?.data || []
    if (query) {
      const q = String(query).toLowerCase()
      list = list.filter(s =>
        String(s.name || '').toLowerCase().includes(q) ||
        String(s.succession_type || '').toLowerCase().includes(q)
      )
    }
