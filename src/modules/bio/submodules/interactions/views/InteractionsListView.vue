...
import InteractionFormModal from '@/modules/bio/submodules/interactions/components/modals/InteractionFormModal.vue'
...
const createModalId = 'bio-interactions-create'
const editModalId = 'bio-interactions-edit'
const deleteModalId = 'bio-interactions-delete'
const isDeleting = ref(false)
const itemToEdit = ref(null)
const itemToDelete = ref(null)

const columns = [
  { field: 'plant_species_title', header: 'Растение', sortable: true },
  { field: 'animal_species_title', header: 'Животное', sortable: true },
  { field: 'site_number', header: 'Площадка', sortable: true },
  { field: 'interaction_type', header: 'Тип', sortable: true },
]
...
const filtered = computed(() => {
  ...
  return items.value.filter((x) =>
    String(x.plant_species || '').toLowerCase().includes(term) ||
    String(x.animal_species || '').toLowerCase().includes(term) ||
    String(x.site || '').toLowerCase().includes(term) ||
    String(x.interaction_type || '').toLowerCase().includes(term)
  )
})
...
const fetchItems = async () => {
  ...
  error.value = null
  try {
    const r = await apiClient.get(endpoints.bio.interactions.interactions)
    const list = r.success ? (r.data.results || r.data || []) : []
    items.value = list.map(x => ({ ...x, id: x.id ?? x.interaction_id ?? x.pk }))
    if (!r.success) error.value = handleApiError(r, 'Ошибка загрузки взаимодействий')
  } catch (e) {
    error.value = handleApiError(e, 'Ошибка загрузки взаимодействий')
  } finally {
...
const handleDelete = async () => {
  ...
  try {
    const r = await apiClient.delete(endpoints.bio.interactions.interactionDetail(itemToDelete.value.id))
    if (r.success) {
      toast.success('Взаимодействие удалено')
      await fetchItems()
      closeById(deleteModalId)
    } else {
      handleApiError(r, 'Ошибка удаления взаимодействия', toast)
    }
  } catch (e) {
    handleApiError(e, 'Ошибка удаления взаимодействия', toast)
  } finally {
...
const handleCreate = async (payload, cb) => {
  try {
    const r = await apiClient.post(endpoints.bio.interactions.interactions, payload)
    if (r.success) {
      ...
      const err = handleApiError(r, 'Ошибка создания взаимодействия', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка создания взаимодействия', toast)
    cb && cb(err)
  }
}
...
const handleUpdate = async (payload, cb) => {
  ...
  try {
    const r = await apiClient.put(endpoints.bio.interactions.interactionDetail(itemToEdit.value.id), payload)
    if (r.success) {
      ...
      const err = handleApiError(r, 'Ошибка обновления взаимодействия', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка обновления взаимодействия', toast)
    cb && cb(err)
  }
}
...
<template>
  <div class="card p-0">
    <div class="card-header custom-card-header border border-bottom-0 text-white d-flex justify-content-between align-items-center">
      <h5 class="mb-0">Взаимодействия (растение-животное)</h5>
      <div class="d-flex gap-2">
        ...
        <button class="btn btn-sm btn-primary text-nowrap" @click="openCreate">
          <Plus :size="16" class="me-1" /> Добавить взаимодействие
        </button>
      </div>
    </div>
    ...
    <div class="card-body">
      ...
      >
        ...
        <template #cell-plant_species_title="{ data }">
          {{ data.plant_species || '-' }}
        </template>
    ...
    <DeleteConfirmModal :modalId="deleteModalId" :title="`Подтвердите удаление взаимодействия`" :message="`Вы уверены, что хотите удалить взаимодействие?`" :loading="isDeleting" warningText="Это действие нельзя будет отменить." @confirm="handleDelete" />

    <InteractionFormModal :modalId="createModalId" @create="handleCreate" />
    <InteractionFormModal :modalId="editModalId" :initialData="itemToEdit" :isEdit="true" @update="handleUpdate" />
  </div>
</template>
