...
import SampleFormModal from '@/modules/bio/submodules/paleobotany/components/modals/SampleFormModal.vue'
...
const createModalId = 'p-samp-create'
const editModalId = 'p-samp-edit'
const deleteModalId = 'p-samp-delete'
...
const columns = [
  ...
  { field: 'collection_date', header: 'Дата', sortable: true },
  { field: 'depth', header: 'Глубина', sortable: true },
...
const filtered = computed(() => {
  ...
  return items.value.filter((x) =>
    String(x.sample_id || '').toLowerCase().includes(term)
  )
})
...
const fetchItems = async () => {
  ...
  error.value = null
  try {
    const r = await apiClient.get(endpoints.bio.paleobotany.samples)
    const list = r.success ? (r.data.results || r.data || []) : []
    items.value = list.map(x => ({ ...x, id: x.id ?? x.pk }))
    if (!r.success) error.value = handleApiError(r, 'Ошибка загрузки образцов')
  } catch (e) {
    error.value = handleApiError(e, 'Ошибка загрузки образцов')
  } finally {
...
const handleDelete = async () => {
  ...
  try {
    const r = await apiClient.delete(endpoints.bio.paleobotany.sampleDetail(itemToDelete.value.id))
    if (r.success) {
      toast.success('Образец удален')
      await fetchItems()
      closeById(deleteModalId)
    } else {
      handleApiError(r, 'Ошибка удаления образца', toast)
    }
  } catch (e) {
    handleApiError(e, 'Ошибка удаления образца', toast)
  } finally {
...
const handleCreate = async (payload, cb) => {
  try {
    const r = await apiClient.post(endpoints.bio.paleobotany.samples, payload)
    if (r.success) {
      toast.success('Образец создан')
      await fetchItems()
      closeById(createModalId)
      cb && cb(null)
    } else {
      const err = handleApiError(r, 'Ошибка создания образца', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка создания образца', toast)
    cb && cb(err)
  }
}
...
const handleUpdate = async (payload, cb) => {
  ...
  try {
    const r = await apiClient.put(endpoints.bio.paleobotany.sampleDetail(itemToEdit.value.id), payload)
    if (r.success) {
      toast.success('Образец обновлен')
      await fetchItems()
      closeById(editModalId)
      cb && cb(null)
    } else {
      const err = handleApiError(r, 'Ошибка обновления образца', toast)
      cb && cb(err)
    }
  } catch (e) {
    const err = handleApiError(e, 'Ошибка обновления образца', toast)
    cb && cb(err)
  }
}
...
<template>
  <div class="card p-0">
    <div class="card-header custom-card-header border border-bottom-0 text-white d-flex justify-content-between align-items-center">
      <h5 class="mb-0">Палеоботаника — образцы</h5>
      <div class="d-flex gap-2">
        ...
        <button class="btn btn-sm btn-primary text-nowrap" @click="openCreate">
          <Plus :size="16" class="me-1" /> Добавить образец
        </button>
      </div>
    </div>
    ...
    <DeleteConfirmModal :modalId="deleteModalId" :title="`Подтвердите удаление образца`" :message="`Вы уверены, что хотите удалить образец?`" :loading="isDeleting" warningText="Это действие нельзя будет отменить." @confirm="handleDelete" />

    <SampleFormModal :modalId="createModalId" @create="handleCreate" />
    <SampleFormModal :modalId="editModalId" :initialData="itemToEdit" :isEdit="true" @update="handleUpdate" />
  </div>
</template>
